...
if (window._dicekill) window._dicekill();

(function () {
  'use strict';

  var INSTANCE = Date.now();
  window._dicekill = function () { INSTANCE = -1; };

  // ── Phase config (matches audit report) ──────────────────────────────────
  var PHASES = [
    { key: 'A', name: 'Random targets',  total: 5000, amount: '0.01', targetFn: function () { return Math.floor(Math.random() * 9800) + 200; }, direction: 'over' },
    { key: 'B', name: 'Target 9800',     total: 1000, amount: '0.01', targetFn: function () { return 9800; }, direction: 'over' },
    { key: 'C', name: 'Custom $10',      total: 200,  amount: '10',   targetFn: function () { return 9700; }, direction: 'over' },
    { key: 'D', name: 'Phase D custom',  total: 500,  amount: '0.01', targetFn: function () { return 9700; }, direction: 'over' },
  ];

  var BETS_PER_EPOCH = 50;
  var BET_DELAY = 500;
  var MAX_ERRORS = 8;
  var DB_NAME = 'dice_capture_v3';
  ...
  // ── Logging ──────────────────────────────────────────────────────────────
  function log(msg) { console.log('%c[dice] ' + msg, 'color:#4fc3f7'); }
  function warn(msg) { console.warn('%c[dice] ' + msg, 'color:#ffb74d'); }
  function good(msg) { console.log('%c[dice] ' + msg, 'color:#81c784'); }
  ...
  function buildPanel() {
    ...
    title.textContent = 'DICE CAPTURE';
    Object.assign(title.style, { color:'#4fc3f7', fontWeight:'700', fontSize:'13px' });
    d.appendChild(title);
  ...
  function placeBet(amount, target, direction) {
    return ensureToken().then(function (token) {
      return api('POST', '/api/v2/dice/bet', {
        amount: String(amount), target: String(target),
        bet_type: direction, currency: 105, security_token: token,
      });
    });
  }
  ...
  function runLoop() {
    ...
    if (phaseIdx >= PHASES.length) {
      good('ALL PHASES COMPLETE - ' + betCount + ' bets, ' + seedCount + ' seeds. Run dice.save()');
    ...
    var target = cfg.targetFn();
    placeBet(cfg.amount, target, cfg.direction).then(function (bet) {
      var r = bet.round || bet;
      var rec = {
        at: new Date().toISOString(), phase: cfg.key,
        request: { amount: cfg.amount, target: target, bet_type: cfg.direction },
        response: {
          id: r.id, result: r.result, is_win: r.is_win,
          amount_currency: r.amount_currency, amount_won: r.amount_won,
          bet_type: r.bet_type, target: r.target, win_chance: r.win_chance,
          multiplier: r.multiplier, nonce: r.nonce,
          drand_round: r.drand_round || null, drand_randomness: r.drand_randomness || null,
          server_seed_hashed: r.server_seed_hashed, client_seed: r.client_seed,
          effective_edge: r.effective_edge, transaction_id: r.transaction_id,
        },
      };
...
console.log('[dice] reference record loaded — see data/ for the captured dataset');
